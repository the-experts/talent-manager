...
export default async function handler(
  ...
  try {
    ...
    const addSkillToColleagueQuery =
      await sql`INSERT INTO colleagues_skills(colleague_id, skill_id, category_id, ability, interest)
    ...
    return response
      .status(200)
      .json({ addSkillToColleagueQuery: addSkillToColleagueQuery });
  } catch (error) {
