...
const Skills = (props: any) => {
  ...
  const fetchedColleagueSkills: ColleagueSkillItem[] | null =
    useFetchColleagueSkills(props?.colleagueId);
  const [allColleagueSkillItems, setAllColleagueSkillItems] = useState<
    ColleagueSkillItem[] | null
  >([]);

  const [techniqueColleagueSkillItems, setTechniqueColleagueSkillItems] =
    useState<ColleagueSkillItem[]>([]);
  const [toolColleagueSkillItems, setToolColleagueSkillItems] = useState<
    ColleagueSkillItem[]
  >([]);
  const [platformColleagueSkillItems, setPlatformColleagueSkillItems] =
    useState<ColleagueSkillItem[]>([]);
  const [languageColleagueSkillItems, setLanguageColleagueSkillItems] =
    useState<ColleagueSkillItem[]>([]);
  const [otherColleagueSkillItems, setOtherColleagueSkillItems] = useState<
    ColleagueSkillItem[]
  >([]);
  ...
  const updateCsiItems = (
    updatedColleagueSkillItemFromCategoryTab: ColleagueSkillItem,
    originalCategoryId: number | undefined,
  ) => {
    if (
      !updatedColleagueSkillItemFromCategoryTab ||
      !allColleagueSkillItems?.length
    ) {
      return;
    }
    const foundIndexOfUpdatedItem = allColleagueSkillItems?.findIndex(
      (csi) => csi.id === updatedColleagueSkillItemFromCategoryTab.id,
    );
    ...
    if (foundIndexOfUpdatedItem > -1) {
      colleagueSkillsCopy[foundIndexOfUpdatedItem] =
        updatedColleagueSkillItemFromCategoryTab;
    } else {
  ...
  const updateTabbedItems = (
    updatedColleagueSkillItem: ColleagueSkillItem,
    categoryListItemsForUpdatedCategoryValue: ColleagueSkillItem[],
    originalCategoryId: number | undefined,
  ) => {
    ...
    if (
      ...
      return updateAbilityInterestValues(
        updatedColleagueSkillItem,
        categoryListItemsForUpdatedCategoryValue,
        updatedColleagueSkillItem.category_id,
      );
      ...
      return updateCategoryAbilityInterestOnColleagueSkillItem(
        originalCategoryId,
        updatedColleagueSkillItem,
      );
    }
  ...
  const updateAbilityInterestValues = (
    updatedCsItemFromCategoryTab: ColleagueSkillItem,
    categoryListItemsForUpdatedCategoryValue: ColleagueSkillItem[],
    categoryId: number,
  ) => {
    const updatedItemIndex = categoryListItemsForUpdatedCategoryValue.findIndex(
      (item) => item.id === updatedCsItemFromCategoryTab.id,
    );
  ...
  const updateCategoryAbilityInterestOnColleagueSkillItem = (
    ...
    const colleagueSkillItemsPreviousCategory =
      getCategorizedColleagueSkillItemsByCategoryId(previousCategoryId);

    // remove item from previous category array
    const updatedColleagueSkillItemsPreviousCategory =
      colleagueSkillItemsPreviousCategory?.filter(
        (csItem) => csItem.id !== updatedColleagueSkillItemId,
      );
    updateTabState(
      previousCategoryId,
      updatedColleagueSkillItemsPreviousCategory,
    );

    // add to new one, with updated values for category, ability, interest
    const categoryIdUpdated: number =
      updatedColleagueSkillItem.category_id as number;
    const colleagueSkillItemsNewCategory =
      getCategorizedColleagueSkillItemsByCategoryId(categoryIdUpdated);
    const indexOfItemToBeUpdated = colleagueSkillItemsNewCategory.findIndex(
      (csItem) => csItem.id === updatedColleagueSkillItem.id,
    );
  ...
  const getCategorizedColleagueSkillItemsByCategoryId = (
    categoryId: number,
  ) => {
    switch (categoryId) {
      case 1:
  ...
  const updateTabState = (
    categoryId: number,
    updatedCategoryTabItems: ColleagueSkillItem[],
  ) => {
    switch (categoryId) {
      case 1:
  ...
  const deleteCsiItem = (itemToBeDeletedId: number) => {
    ...
    const itemToBeDeleted = allColleagueSkillItems.find(
      (allCsItem) => allCsItem.id === itemToBeDeletedId,
    );
    const updatedColleagueSkills = allColleagueSkillItems?.filter(
      (csi) => csi.id !== itemToBeDeletedId,
    );
    ...
    const categorizedCsItems = getCategorizedColleagueSkillItemsByCategoryId(
      itemToBeDeleted?.category_id,
    );
    const updatedCategorizedColleagueSkills = categorizedCsItems?.filter(
      (categorizedCsItem) => categorizedCsItem.id !== itemToBeDeletedId,
    );
    updateTabState(
      itemToBeDeleted?.category_id,
      updatedCategorizedColleagueSkills,
    );
  };
  ...
  const sortSkillsByCategory = (colleagueSkillItems: ColleagueSkillItem[]) => {
    ...
    const fetchedColleagueSkillsToBeUpdated = fetchedColleagueSkills
      ? [...fetchedColleagueSkills]
      : null;

    colleagueSkillItems.forEach((colleagueSkillItem: ColleagueSkillItem) => {
      const foundIndex = fetchedColleagueSkillsToBeUpdated?.findIndex(
        (it: ColleagueSkillItem) => it.id === colleagueSkillItem.id,
      );
    ...
    fetchedColleagueSkillsToBeUpdated?.forEach((colleagueSkillItem) => {
      colleagueSkillItem.key =
        colleagueSkillItem?.key ?? colleagueSkillItems.length + 1;
          ...
          <Tab as={Fragment}>
            {({ selected }) => (
              <button
                className={
                  selected
                    ? 'flex-1 p-1 w-40 text-white'
                    : 'flex-1 p-1 w-40 bg-white text-black'
                }
              >
                All
              </button>
