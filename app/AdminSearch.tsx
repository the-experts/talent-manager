...
const AdminSearch = (props: any): JSX.Element => {
    const [searchType, setSearchType] = useState(1);
    const [searchTerm, setSearchTerm] = useState<string>('');
    const [searchResults, setSearchResults] = useState<any[]>([]);
    const [searchFieldPlaceholder, setSearchFieldPlaceholder] = useState<string>('Enter skill name');
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const res = await axios.get(`/api/fetch-colleague-skill-search-data?searchTerm=${searchTerm}&searchType=${searchType}`)
                    .then(response => {
    ...
    const onRadioButtonChange = (e: RadioChangeEvent) => {
        ...
        radioSelectionInput === 1 ? setSearchFieldPlaceholder('Enter skill name') : setSearchFieldPlaceholder('Enter colleague name')
    };
    ...
    const onSearchInputChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        return (
            <>
                ...
                <Radio.Group name="radiogroup" className={'w-full ml-3 mt-3'} onChange={onRadioButtonChange} value={searchType}>
                    <Radio value={1}>Skill name</Radio>
                    <Radio value={2}>Colleague name</Radio>
                </Radio.Group>
                <Input placeholder={searchFieldPlaceholder} onChange={onSearchInputChange} value={searchTerm} className={'ml-3 mt-3 w-80'} />
                <Table
