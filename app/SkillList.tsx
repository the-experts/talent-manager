...
const EditableCell: React.FC<EditableCellProps> = ({
  ...
  if (record?.category?.title) {
    ...
    const foundCategory = categories?.find(
      (category) => category.id === record.category_id,
    );
    if (foundCategory) {
  ...
  if (node === '' || node === undefined) {
    if (record?.skill_id && allSkills.length > -1) {
      const findExistingSkill = allSkills?.find(
        (skill) => skill?.id === record?.skill_id,
      );
  ...
  const handleSelectChange = (event: any) => {
    const foundDisplayCategory = categories.find(
...
const SkillList = ({
  ...
  let [currentColleagueSkills, setCurrentColleagueSkills] = useState<
    ColleagueSkillItem[]
  >([]);
  let categoryName: string;

  props?.colleagueSkills?.forEach(
    (existingColleagueSkill: ColleagueSkillItem) => {
      if (props?.categories && existingColleagueSkill?.category_id) {
        const foundCategory = props.categories?.find(
          (category: Category) =>
            category.id === existingColleagueSkill?.category_id,
        );
  ...
  const cancel = () => {
    const currentSkill = currentColleagueSkills.find(
      (currentColleagueSkill) => currentColleagueSkill.key === editingKey,
    );
  ...
  const saveColleagueSkillItem = async (
    skillId: number,
    editedItem: EditedItem,
    isEditing = false,
  ): Promise<any> => {
    const url = isEditing
      ? '/api/edit-colleague-skill'
      : '/api/add-skill-to-colleague';
  ...
  const saveNewColleagueSkillItem = async (
    newSkillResponseId: number,
    index: number,
    updatedCurrentColleagueSkills: ColleagueSkillItem[],
    editedItem: EditedItem,
    ...
    const response = await saveColleagueSkillItem(newSkillResponseId, {
      ...
      console.error(
        'failed saving new colleague-skill-item with existing skill:',
        error,
      );
    });

    const newCsiResponseItem = response?.data?.addSkillToColleagueQuery?.rows
      ? response?.data?.addSkillToColleagueQuery?.rows[0]
      : undefined;

    if (!newCsiResponseItem) {
      throw new Error(
        'new colleague-skill-item could not be retrieved from server',
      );
    }

    const updatedIndex = updatedCurrentColleagueSkills.findIndex(
      (colleagueSkillItem: ColleagueSkillItem) =>
        colleagueSkillItem.id === newCsiResponseItem.id,
    );
    if (updatedIndex) {
      let newCsi = {
  ...
  const editColleagueSkillItem = async (
    editedItem: EditedItem,
    updatedCurrentColleagueSkills: ColleagueSkillItem[],
    originalCategoryId: number | undefined,
  ) => {
    ...
    await saveColleagueSkillItem(editedItem.skill_id, editedItem, true)
      .then((response) => {
        const updatedItem = response?.data?.editColleagueSkillQuery?.rows
          ? response?.data?.editColleagueSkillQuery?.rows[0]
          : undefined;
        if (!updatedItem) {
          return;
        }
        const updatedIndex = updatedCurrentColleagueSkills.findIndex(
          (colleagueSkillItem: ColleagueSkillItem) =>
            colleagueSkillItem.id === updatedItem.id,
        );
        const foundCategory = props.categories?.find(
          (category: Category) => category.id === editedItem?.category?.value,
        );
  ...
  const save = async (key: React.Key) => {
    try {
      ...
      const index = updatedCurrentColleagueSkills.findIndex(
        (item) => key === item.key,
      );
      ...
      if (updatedCurrentColleagueSkills[index]) {
        originalCategoryId =
          updatedCurrentColleagueSkills[index]?.category_id ?? undefined;
      }
      ...
      if (row?.name) {
        const matchingSkill = props.allSkills.find(
          (allSkillItem: SkillItem) => allSkillItem.name === row.name,
        );
      ...
      if (editedItem.skill_id === undefined && editedItem.id === undefined) {
        ...
        await axios
          .post('/api/create-skill', { newSkill })
          .then(async (response) => {
            const newSkillResponse = response?.data?.createSkillQuery?.rows[0];
            if (!newSkillResponse) {
              throw new Error('new skill not saved');
            }
            const updatedColleagueSkillItems = await saveNewColleagueSkillItem(
              newSkillResponse.id,
              index,
              updatedCurrentColleagueSkills,
              editedItem,
              originalCategoryId,
            );
            updatedColleagueSkillItems
              ? (currentColleagueSkills = updatedColleagueSkillItems)
              : null;
            const newSkill = {
      ...
      // skill already exists, colleagueSkillItem is being newly created
        ...
        await saveNewColleagueSkillItem(
          editedItem.skill_id,
          index,
          updatedCurrentColleagueSkills,
          editedItem,
      ...
      // skill id and item id already defined, edit existing entry
        ...
        const updatedColleagueSkillItems = await editColleagueSkillItem(
          editedItem,
          updatedCurrentColleagueSkills,
          originalCategoryId,
        );
      ...
      render: (_: any, record: ColleagueSkillItem) => {
        ...
        return editable && recordKey ? (
          ...
          <>
            <Typography.Link
              disabled={editingKey !== ''}
              onClick={() => edit({ ...record, key: recordKey })}
            >
              Edit
            </Typography.Link>{' '}
            <Typography.Link
              disabled={editingKey !== ''}
              onClick={() => handleDelete(recordKey)}
            >
              Delete
            </Typography.Link>
          </>
  ...
  const handleDelete = async (key: React.Key | undefined) => {
    ...
    const itemToBeDeleted = currentColleagueSkills.find(
      (item) => item.key === key,
    );
    if (itemToBeDeleted?.id) {
      const colleagueSkillId = itemToBeDeleted?.id;
      return await axios
        .post('/api/delete-colleague-skill', { colleagueSkillId })
        .then((response) => {
          const newData = currentColleagueSkills.filter(
            (item) => item.key !== key,
          );
  ...
  const handleAdd = () => {
    ...
    setTimeout(() => {
      document
        .querySelector('tr[data-row-key]:last-of-type')
